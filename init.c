...
{
    t_FractalBounds *bounds = (t_FractalBounds *)malloc(sizeof(t_FractalBounds));
    if (bounds == NULL)
        return (NULL);
    bounds->min_r = min_r;
    bounds->max_r = max_r;
...
{
    t_RenderData *render_data = (t_RenderData *)malloc(sizeof(t_RenderData));
    if (render_data == NULL)
        return NULL;
...
{
    t_W_R_D *w_r_d = (t_W_R_D *)malloc(sizeof(t_W_R_D));
    if (w_r_d == NULL)
        return NULL;
    w_r_d->window = window;
